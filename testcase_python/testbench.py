...
TRACING_MODE = None # disable trace compare/generate for lab environment
...
def runCocotbTestsProcessor(pytestconfig):
    ...
    # --- CẤU HÌNH DÙNG ICARUS VERILOG ---
    sim_runner = "icarus"

    # Các cờ biên dịch cho Icarus
    icarus_build_args = [
    ...
    # Lấy danh sách test từ tham số dòng lệnh (nếu có), nếu không thì chạy tất cả (None)
    tests_to_run = getattr(pytestconfig.option, 'tests', None)
...
async def riscvTest(dut, binaryPath=None, tracingMode=None):
    ...
    for cycles in range(TIMEOUT_CYCLES):
        ...
        # ĐÃ SỬA: Comment dòng này lại để tránh lỗi thiếu file cycle_status.v
