...
This repository contains the RTL implementation of a 5-stage Pipelined RISC-V Processor targeting the Arty-Z7 FPGA board.
The core implements the RV32IM instruction set (Integer + Multiplication/Division) and is designed to handle complex pipeline hazards including data forwarding and control flushing.
...
Copy all files from inside /rtl

Copy all files from inside /testbench

Copy all files from inside /testcase_python

Copy necessary environment files from /venv
...
The verification environment uses Cocotb to drive the Verilog simulation through Python.

1. runCocotbTestsProcessor

Main entry point that configures and launches the simulation.

Uses -g2012 to enable SystemVerilog features.

Passes the DIVIDER_STAGES macro to match the hardware divider configuration.

Automatically compiles DatapathPipelined.v and its dependencies.

2. preTestSetup

Ensures every test starts from a known state.

Generates a 4ns clock.

Asserts reset for one cycle.

Loads RISC-V binaries into memory using riscv_binary_utils.

3. rvTestFactory

Factory pattern for scalable test generation.

Iterates through RV_TEST_BINARIES.

Automatically creates test cases such as riscvTest_033.

Enables full ISA test suite execution with a single command.
